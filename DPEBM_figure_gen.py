...
from model3 import ExplainableBoostingMachine, DPExplainableBoostingMachine
...
def experiment_2(X, y, feature_names):
    ...
    ebm_dp4 = DPExplainableBoostingMachine(task="classification", epsilon=4.0, delta=1e-6)
    ebm_dp4.fit(X, y)
    centers_dp4, sf_dp4 = ebm_dp4.explain_feature(age_idx)
    
    # Step 6: Train DPEBM with eps=1
    print("Training DPEBM (eps=1)...")
    ebm_dp1 = DPExplainableBoostingMachine(task="classification", epsilon=1.0, delta=1e-6)
    ebm_dp1.fit(X, y)
...
def experiment_3(ebm_dp1, centers_dp1, sf_dp1, feature_names):
    ...
    axes[2].step(centers_dp1, sf_repaired, where='mid', color='purple', linewidth=2, label='Repaired')
...
def experiment_4(X, y, feature_names):
    ...
    dp_eps4 = DPExplainableBoostingMachine(task="classification", epsilon=4.0, delta=1e-6)
    dp_eps4.fit(X, y)
    
    print("Training DPEBM (ε=1)...")
    dp_eps1 = DPExplainableBoostingMachine(task="classification", epsilon=1.0, delta=1e-6)
    dp_eps1.fit(X, y)
